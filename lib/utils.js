...
class Utils {
  ...
  getTimeToNextEpisode(time, scale = 'ms') {
    if (scale == 'ms') {
  ...
  getAiringDate(startDate) {
    if (startDate.day) {
      return `${this.MONTHS[startDate.month]} ${startDate.day}, ${startDate.year}`
    } else if (startDate.month) {
      return `${this.MONTHS[startDate.month]}, ${startDate.year}`
    }
